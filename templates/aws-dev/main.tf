# =============================================================================
# AWS Dev — Kubernetes Development Template
# =============================================================================
# AWS-focused dev environment with Kiro as the primary AI agent.
#
# Included tools:
...
#
# Authentication:
#   Kiro authenticates independently via AWS Builder ID, IAM Identity Center,
#   GitHub, or Google. No AI Bridge or external API keys are needed.
#   Users run `kiro-cli login` interactively or pre-configure credentials.
#
# AWS tooling:
#   AWS CLI is pre-installed in the base image. AWS CDK is installed via npm
#   in the startup script. Configure AWS credentials via `aws configure` or
...
# -----------------------------------------------------------------------------
# Agent — startup script installs AWS tooling and Kiro CLI
# -----------------------------------------------------------------------------
...
resource "coder_agent" "main" {
  ...
  #   1. Adds npm global bin to PATH
  #   2. Installs AWS CDK CLI via npm
  #   3. Installs Kiro CLI via curl
  startup_script = <<-EOT
    #!/bin/bash
    touch ~/.bashrc
