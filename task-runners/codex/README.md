# Codex Tasks — Task Runner Template

Ephemeral task runner for executing prompts from the Coder Tasks UI via OpenAI Codex on Kubernetes.

## Task Runner vs Workspace

Unlike workspace templates (e.g. `ai-dev`), task runners are designed to receive a prompt from the Tasks UI and execute it autonomously. They use `coder_ai_task` + `data.coder_task` resources to wire the Tasks UI to the Codex agent.

## Included Apps

| App | Description |
|---|---|
| **Codex** | OpenAI coding agent (registry module — handles install, AgentAPI, web UI, task reporting) |
...
The `codex` registry module handles `OPENAI_API_KEY` and `CODER_AIBRIDGE_SESSION_TOKEN` configuration via AI Bridge.
